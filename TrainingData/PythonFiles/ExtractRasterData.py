...
SplitCP = CompletePath.split('\\')
DesiredInfo2 = SplitCP[0:-1]
CompletePath2 = ''
i = 0
for aItem in DesiredInfo2:
	if i>0:
		CompletePath2 = CompletePath2+'\\'+aItem
	if i==0:
		CompletePath2 = aItem
	i = i+1
# Identify basic file paths
TCI = CompletePath+'/TIF_Code_Images/'
PLD = CompletePath+'/ParsedSegmentData/'
SHP_Loc = CompletePath+'/Shapefile/'
# We want the analysis meta file from the training data
# ti contains all the relevant info for the training data set
SegMeta = CompletePath+'/Analysis_Files/Analysis_Meta.txt'
MetaLoc = CompletePath+'/Analysis_Files/'
...
BasePaths = list(os.listdir(CompletePath+'/TIF_Code_Images/'))

with rasterio.Env():
	for aPath in BasePaths:
		Files = list(os.listdir(CompletePath+'/TIF_Code_Images/'+aPath))
		for File in Files:
			FP = CompletePath+'/TIF_Code_Images/'+aPath+'/'+File
...
# To check if needed
df = pd.DataFrame(NSR)
df.to_csv(MetaLoc+'NSR.csv')
df=None

# ExtentLoc no longer needed, close it out
ExtentLoc = None
t = []
with open(MetaLoc+'NSR.csv', 'r') as File:
	for M in File:
		M = M.replace('\n', '')
		Sp1 = M.split(',')
...
for i, pt in enumerate(points):
	...
	try:
		...
		try:
			...
			ConvDate = datetime.strptime(EstDate, '%m/%d/%Y')
		...
		try:
			...
			DateFnd = datetime.strptime(DateFnd, '%m/%d/%Y')
...
df.to_csv(MetaLoc+'SegImagePaths.csv')
df = None
		...
		if Segment == NestSegment:
			...
			for Image in ImagePaths:
				...
				try:
					...
					Date = datetime.strptime(DateCode, '%Y%m%d')
...
df.to_csv(MetaLoc+'NestImInfo.csv')
df = []
# sys.exit()
...
with open(MetaLoc+'NestImInfo.csv', 'r') as File:
	for aItem in File:
		aItem = aItem.replace('\n', '')
		Sp1 = aItem.split(',')
...
# for aItem in NestPixQua:
	...
	# Loop through the selected nests and extract the coverage class
	...
	# MOD THIS SECTION TO ALLOW NDVI AT SATURATED AREAS
	# Define nests that NDVI can be conducted on. We will allow for land
	# as well as the interference for saturation

	## GOOD NESTS GOOD NESTS
	## GOOD NESTS GOOD NESTS 

	# Basically the same as habitable zone. If Water is near by before
	# any other interference, then the record is good for ndvi data.
	# There are large amounts of shoreline littered with 352 and 480.
	# This is most probably due to saturated sand, the reason for 
	# a saturated pixel hopefully ; )

	# if ptslist[0]==322 or ptslist[0]==352 or ptslist[0]==480:
	# 	# Only run if point is identified on land
	# # Mod for saturated areas 9/27/2020
	# # if ptslist[0]==322 or ptslist[0]==352 or ptslist[0]==480:
	# 	Continue = 0
	# 	TempDist = []
	...
	# 	# GoodNests.append([aItem[0], TempDist[0]])
	# 	# print DistMax
...
DistDf.to_csv(MetaLoc+'NestDistance.csv')

# GoodDf = pd.DataFrame(GoodNests, columns = ['Nest Id', 'Distance'])

# GoodDf.to_csv(MetaLoc+'NestGood.csv')
...
# with open(MetaLoc+'NestDistance.csv', 'r') as File:
# 	for aRow in File:
# 		RawDist.append(aRow)
# MOD
with open(MetaLoc+'NestDistance.csv', 'r') as File:
...
NDVIdf.to_csv(MetaLoc+'NestNDVI.csv')
...
# 	# Extract neighboring NDVI values

# 	Continue = 0
# 	TempDist = []
# 	# Incr set to 90, will get all neighbouring pixels
# 	Incr = 90
# 	for index, row in pts.iterrows():
# 		ID = int(row.NEST_ID)
# 		PointBuff = row.geometry.buffer(Incr)
...
# 		# Convert the clipped tif to a polygon file, select the polygon (pixel)
# 		# the nest is in, then select the neighboring polygons (pixels). These
# 		# are the pixels we are interested in for the analysis. Will allow for
# 		# increased accuracy with RandomForest

# 		gdal.UseExceptions()
# 		#  get raster datasource
# 		src_ds = gdal.Open('Analysis_Files/WorkingClipFile.tif')
# 		if src_ds is None:
...
# 		dst_layername = 'Data'
# 		drv = ogr.GetDriverByName("ESRI Shapefile")
# 		dst_ds = drv.CreateDataSource('Analysis_Files/WorkingClipPolyFile.shp')
# 		dst_layer = dst_ds.CreateLayer(dst_layername, srs = srs)
