...
def SelectScenes(Option):
	# If using this via a training funciton, location of files and folders
	# will be different in comparison to where the file is being
	# pulled from for use.
	if Option == 'Train':
		# Add folder paths
		...
		sys.path.append('ParsedSegmentData')
		Par_Dat = 'ParsedSegmentData/'
		OutInAnalysis = 'Analysis_Files/Analysis_Meta.txt'
		
	if Option == 'Predict':
		# Define outer directory
		dir_path = str(os.path.dirname(os.path.realpath(__file__)))
		Sp1 = dir_path.split('\\')
		NewDir = ''
		NeededComponents = Sp1[0:-2]
		i = 0
		for aString in NeededComponents:
			if i == 0:
				# First iteration
				NewDir=aString
			if i>0:
				NewDir=NewDir+'/'+aString
			i = i+1

		Par_Dat = NewDir+'/ParsedSegmentData/'
		OutInAnalysis = NewDir+'/Analysis_Files/Analysis_Meta.txt'
		SF_Dir = NewDir+'/TrainingData/Shapefile/'

	#################################################################
	# Selecting section
	RawID = []
	with open(OutInAnalysis, 'r') as Doc:
		for aItem in Doc:
			RawID.append(aItem)
	RawID = RawID[0].split(',')
	...
	with open(OutInAnalysis, 'a') as Doc:
