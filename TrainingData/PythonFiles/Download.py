...
def main(username, email, order, target_directory, password=None, host=None, verbose=False,
    ...
    with Api(username, password, host) as api:
        ...
        for o in orders:
            ...
            print '\nDownloading files...\n'
            for s in tqdm.tqdm(range(ProgBarLimit)):
            # for s in range(len(scenes)):
                LOGGER.info('File {0} of {1} for order: {2}'.format(s, len(scenes), o))
...
for aItem in DesiredInfo:
	...
	if i==0:
		CompletePath = aItem
	i = i+1
dir = CompletePath+'/Raw_Images'
if not os.path.exists(dir):
    os.makedirs(dir)
OutputLoc = CompletePath+'/Raw_Images/'


username = 'EnviBio'
email = '201306381@panthers.greenville.edu'
target_directory = OutputLoc
password = 'Marine613'
# order = 'espa-201306381@panthers.greenville.edu-09142020-143659-909'
order ='espa-201306381@panthers.greenville.edu-09222020-110810-127'
# sys.exit()
# 
main(username, email, order, target_directory, password=password, host=None, verbose=False,
    checksum=False, retry=0, no_order_directories=False)
